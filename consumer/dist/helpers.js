export function calculateTranscodingSpeedScore(metrics, transcodeInfo, videoPlaybackMetric) {
...
export function calculateOutputQualityScore(metrics, transcodeInfo) {
    ...
    if (outputCount >= expectedOutputs)
        return 100;
    else if (outputCount >= expectedOutputs * 0.75)
        return 75;
    else if (outputCount >= expectedOutputs * 0.5)
        return 50;
    else
        return 25;
}
export function calculateMemoryEfficiencyScore(metrics) {
...
export function calculateCPUEfficiencyScore(metrics) {
...
export function stabilityScore(metrics) {
    const cpuValues = metrics.map(m => m.cpu_percent).filter((v) => typeof v === 'number');
    if (cpuValues.length === 0)
        return 0;
...
export function getQoeCategory(qoe) {
    if (qoe >= 85)
        return 'Excellent';
    else if (qoe >= 70)
        return 'Good';
