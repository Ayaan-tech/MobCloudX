...
function calculateAudioQualityScore(metrics: TelemetryMetrics[]): number {
    const audioMetrics = metrics.filter(
        (m) =>
            typeof m.audio_jitter_ms === 'number' ||
            typeof m.audio_packet_loss_pct === 'number' ||
            typeof m.av_sync_offset_ms === 'number'
    );

    if (audioMetrics.length === 0) return 70;
    ...
    for (const m of audioMetrics) {
        ...
        if (jitter > 80) score -= 8;
        else if (jitter > 40) score -= 4;

        if (loss > 3) score -= 10;
        else if (loss > 1) score -= 5;

        if (avSync > 120) score -= 6;
    }
...
export class QoeCalculator{
    ...
    private sessionVMAFScores: Map<string, { score: number; resolution: string }[]> = new Map();
    ...
    addVMAFScore(sessionId: string, score: number, resolution: string){
