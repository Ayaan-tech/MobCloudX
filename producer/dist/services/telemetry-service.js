...
postRoutes.post('/transcode-event', zValidator("json", TrancodeEventSchema), async (c) => {
    const body = await c.req.json();
    try {
        const payload = typeof body === 'string' ? JSON.parse(body) : body;
...
postRoutes.post('/qoe-score', zValidator("json", QoeSchema), async (c) => {
    const body = await c.req.json();
    try {
        const payload = typeof body === 'string' ? JSON.parse(body) : body;
 ...
 */
postRoutes.post('/adaptation/decision/:sessionId', zValidator('json', AdaptationDecisionSchema), async (c) => {
    const sessionId = c.req.param('sessionId');
    const body = await c.req.json();
    try {
        const payload = typeof body === 'string' ? JSON.parse(body) : body;
 ...
 */
postRoutes.post('/adaptation/feedback', zValidator('json', AdaptationFeedbackSchema), async (c) => {
    const body = await c.req.json();
    try {
        const payload = typeof body === 'string' ? JSON.parse(body) : body;
        const feedback = {
 ...
 */
postRoutes.post('/vmaf-score', zValidator('json', VMAFScoreSchema), async (c) => {
    const body = await c.req.json();
    try {
        const payload = typeof body === 'string' ? JSON.parse(body) : body;
        const vmafData = {
